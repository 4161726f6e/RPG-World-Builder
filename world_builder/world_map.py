...
"""
def map_main():
    # Adjust this width to alter the size of the map.
    # 2000 makes a decently sized map but can produce
    #  an error of "IndexError: index 2000 is out of 
    #  bounds for axis 0 with size 2000"
    #
    # 4000 is the original value and while it doesn't
    #  seem to produce index errors, it is big
    im_width = 2000
    ...
    def place_city_name(colors, x,y):
        def get_city_name():
            ...
            city_names_used.append(name)
            img = Image.new('RGB', (200, 30), color = (0,0,0))
            fnt = ImageFont.truetype('map_assets/akaPotsley.ttf', 28)
            d = ImageDraw.Draw(img)
            d.text((0,0), name, font=fnt, fill=(255,255,255))
            img.save('map_assets/town_name.png')
            name_im = Image.open("map_assets/town_name.png")
            return name_im.load()
        name = get_city_name()
    ...
    def place_town_name(colors, x,y):
        def get_town_name():
            ...
            while name_taken(name):
                ...
                name = start + end
            names_used.append(name)
            town_names_used.append(name)
            img = Image.new('RGB', (200, 30), color = (0,0,0))
            fnt = ImageFont.truetype('map_assets/akaPotsley.ttf', 24)
            d = ImageDraw.Draw(img)
            d.text((0,0), name, font=fnt, fill=(255,255,255))
            img.save('map_assets/town_name.png')
            name_im = Image.open("map_assets/town_name.png")
    ...
    water_back = Image.open("map_assets/water_back.png")
    water_back = water_back.resize((im_width,im_width))
    w_back = water_back.load()

    hill_back = Image.open("map_assets/hill_text.png")
    hill_back = hill_back.resize((im_width,im_width))
    h_back = hill_back.load()

    tree_im1 = Image.open("map_assets/tree1.png")
    tree1 = tree_im1.load()
    tree_im2 = Image.open("map_assets/tree2.png")
    tree2 = tree_im2.load()
    tree_im3 = Image.open("map_assets/tree3.png")
    tree3 = tree_im3.load()
    tree_im4 = Image.open("map_assets/tree4.png")
    tree4 = tree_im4.load()
    tree_im5 = Image.open("map_assets/tree5.png")
    tree5 = tree_im5.load()

    city_im = Image.open("map_assets/city.png")
    city = city_im.load()

    town_im = Image.open("map_assets/town.png")
    town= town_im.load()

    ruin_im = Image.open("map_assets/ruins.png")
    ruins = ruin_im.load()

    cave_im = Image.open("map_assets/cave.png")
    cave = cave_im.load()

    camp_im = Image.open("map_assets/camp.png")
    camp = camp_im.load()



    mount_img1 = Image.open("map_assets/mount1.png")
    mount1= mount_img1.load()
    mount_img2 = Image.open("map_assets/mount2.png")
    mount2= mount_img2.load()
    mount_img3 = Image.open("map_assets/mount3.png")
    mount3= mount_img3.load()

    mount_img4 = Image.open("map_assets/mount1.png")
    mount_img4 = mount_img4.resize((14,14))
    mount4= mount_img4.load()
    mount_img5 = Image.open("map_assets/mount2.png")
    mount_img5 = mount_img5.resize((14,14))
    mount5= mount_img5.load()
    mount_img6 = Image.open("map_assets/mount3.png")
    mount_img6 = mount_img6.resize((14,14))
    mount6= mount_img6.load()

    swamp_im = Image.open("map_assets/swamp.png")
    ...
    with open('map_assets/city_names.txt', 'w') as fp:
        for item in city_names_used:
            if item == city_names_used[-1]:
                fp.write("%s" % item)
            else:
                fp.write("%s\n" % item)

    with open('map_assets/town_names.txt', 'w') as fp:
        for item in town_names_used:
            if item == town_names_used[-1]:
                fp.write("%s" % item)
            else:
                fp.write("%s\n" % item)

    with open('map_assets/cave_names.txt', 'w') as fp:
        for item in cave_names_used:
            if item == cave_names_used[-1]:
                fp.write("%s" % item)
            else:
                fp.write("%s\n" % item)

    with open('map_assets/camp_names.txt', 'w') as fp:
        for item in camp_names_used:
            if item == camp_names_used[-1]:
                fp.write("%s" % item)
            else:
                fp.write("%s\n" % item)

    with open('map_assets/ruin_names.txt', 'w') as fp:
        for item in ruin_names_used:
            if item == ruin_names_used[-1]:
                fp.write("%s" % item)
            else:
                fp.write("%s\n" % item)
...
if __name__ == "__main__":
    ...
    # Try making the map, catch index error
    #  and try again.
    #
    # Adjust the im_width variable at the top
    #  to prevent the error, but makes a big map
    while True:
        try:
            map_main()
        except IndexError:
